...
package trace // import "github.com/open-telemetry/opentelemetry-collector-contrib/receiver/skywalkingreceiver-cola/internal/trace"
...
type Receiver struct {
	...
	segmentObserver *profile.SegmentCache
	agent.UnimplementedTraceSegmentReportServiceServer
}

// SetSegmentObserver records segment_id → trace id for profile snapshot correlation.
func (r *Receiver) SetSegmentObserver(cache *profile.SegmentCache) {
	r.segmentObserver = cache
}
...
func (r *Receiver) observeSegment(segment *agent.SegmentObject, ptd ptrace.Traces) {
	if r.segmentObserver == nil {
		return
	}
	ident := profile.SegmentIdent{
		SWTraceID:       segment.GetTraceId(),
		Service:         segment.GetService(),
		ServiceInstance: segment.GetServiceInstance(),
	}
	if ptd.ResourceSpans().Len() > 0 {
		scopes := ptd.ResourceSpans().At(0).ScopeSpans()
		if scopes.Len() > 0 && scopes.At(0).Spans().Len() > 0 {
			tid := scopes.At(0).Spans().At(0).TraceID()
			ident.OTelTraceID = hex.EncodeToString(tid[:])
		}
	}
	r.segmentObserver.Put(segment.GetTraceSegmentId(), ident)
}
